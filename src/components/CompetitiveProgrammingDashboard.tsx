  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
      <div className="flex items-center gap-3 mb-8">
        ...
        <h2 className="text-3xl font-bold text-gray-900">Competitive Programming</h2>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {ratings.length === 0 ? (
            <div className="col-span-3 text-center py-12 text-gray-500">
              ...
              <p className="text-lg">No ratings recorded yet</p>
            </div>
          ) : (
            ratings.map((rating) => (
              <div 
                key={rating.platform} 
                className="relative overflow-hidden bg-gradient-to-br from-gray-50 to-white border border-gray-200 rounded-xl p-6 hover:shadow-lg transition-all group"
              >
                <div className={`absolute top-0 right-0 w-32 h-32 bg-gradient-to-br ${getPlatformGradient(rating.platform)} opacity-10 rounded-full -mr-16 -mt-16`}></div>
                
                <div className="relative">
                  <div className="flex items-center justify-between mb-3">
                    <h4 className="font-bold text-lg text-gray-800">{rating.platform}</h4>
                  ...
                  {editingPlatform === rating.platform ? (
                    <input
                      ...
                      className="w-full px-3 py-2 border-2 border-blue-300 rounded-lg text-2xl font-bold focus:outline-none focus:ring-2 focus:ring-blue-500"
                      autoFocus
                    />
                  ) : (
                    <p className={`text-4xl font-bold bg-gradient-to-r ${getPlatformGradient(rating.platform)} bg-clip-text text-transparent mb-2`}>
                      {rating.rating}
                    </p>
                  )}
        ...
        <div className="space-y-4">
          {recentContests.length === 0 ? (
            <div className="text-center py-12 text-gray-500">
              ...
              <p className="text-lg">No contest logs yet</p>
            </div>
          ) : (
              ...
              >
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <span className={`px-3 py-1 bg-gradient-to-r ${getPlatformGradient(contest.platform)} text-white text-xs font-bold rounded-full`}>
                      {contest.platform}
                    </span>
                    <span className="font-semibold text-gray-800">{contest.contest_name}</span>
                  </div>
      ...
      <div className="mt-10 pt-8 border-t border-gray-200">
        <h3 className="text-xl font-bold text-gray-800 mb-5">Contest Schedule</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            { name: 'Codeforces Div 2', time: 'Fri-Sun at 8:05 PM IST', color: 'blue' },
            { name: 'LeetCode Weekly', time: 'Every Sunday at 8:00 AM IST', color: 'yellow' },
            { name: 'LeetCode Biweekly', time: 'Every 2 weeks Sat at 8:00 PM IST', color: 'orange' },
            { name: 'CodeChef Starters', time: 'Every Wednesday at 8:00 PM IST', color: 'amber' }
          ].map((schedule, idx) => (
